...
- **Single Node 프로세스, 포트 4483.** Next.js 16 App Router, `/api/**` 는 Node runtime. Python 은 `packages/skills/` 안에서 per-call subprocess 로만 등장.
...
- **Long-lived state (MCP manager, 엔진 run registry, scheduler, DB connection) 는 `globalThis` 에.** Next dev HMR 이 subprocess 누수나 싱글톤 중복을 만들지 않게.
...
- `pnpm dev` / `pnpm build` / `pnpm start` — Next.js 서버 (4483, 스케줄러 + 엔진 포함)
