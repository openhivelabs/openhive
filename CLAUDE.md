...
### Runtime tuning env (S1-S4 + A1-A4)
...
| `OPENHIVE_DEBUG_RESULT_CAP` | `0` | S1 — debug log per cap |
| `OPENHIVE_MICROCOMPACT_STALE_MS` | `300000` | S2 — stale threshold (5min) |
| `OPENHIVE_MICROCOMPACT_DISABLED` | `0` | S2 — kill switch |
| `OPENHIVE_MICROCOMPACT_MIN_CHARS` | `200` | S2 — skip-below threshold |
| `OPENHIVE_FORK_DISABLE` | `0` | S3 — parallel fork kill switch |
| `OPENHIVE_LEDGER_DISABLED` | `0` | S4 — ledger write/read no-op |
...
| `OPENHIVE_TOOL_PARTITION_V2` | `1` | A1 — `0` = legacy splitToolRuns |
| `OPENHIVE_HOOKS_DISABLED` | `0` | A2 — config 파싱 skip |
...
- **Microcompact.** Lead 의 `externalHistory` 에서 마지막 assistant 후 `OPENHIVE_MICROCOMPACT_STALE_MS` (기본 5분) 경과 시 read-only tool_result 본문을 in-place 비움. trajectory 도구 (`delegate_to`, `ask_user`, todo, `sql_exec`, `activate_skill`) 는 절대 보존. (S2)
...
- **Hooks.** `~/.openhive/config.yaml#hooks` 에 SessionStart / PreToolUse / Stop 셸 hook 등록 가능. exit 2 또는 stdout `{decision:'block'}` 로 tool 차단. spawn 비용 인지하고 `*` matcher 사용. `OPENHIVE_HOOKS_DISABLED=1` 로 완전 비활성. (A2)
- **Artifact rehydration.** 산출물은 `artifact://session/{id}/artifacts/{rel}` URI. `read_artifact` 도구로 텍스트 본문 (`OPENHIVE_ARTIFACT_READ_MAX_CHARS` 기본 50_000자) 또는 메타 회수. path traversal / cross-session 거부, 바이너리 mime 은 메타만. (A3)
- **Token math.** `effectiveWindow(provider, model)` 가 모든 압축/차단 임계값의 SSoT. char 추정 + 권위 input_tokens 결합. drift > 25% 면 `token.estimate.drift` 이벤트. `OPENHIVE_BLOCK_ON_OVERFLOW=1` 로 turn 차단 활성화. (A4)
